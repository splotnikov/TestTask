$(document).ready(function() {
	...
	var loadProject = function(categoryId, projectId) {
		...
		$.get('projects/' + projectId, function(project) {
			$('#project').html('<div>name: ' + project.title + '</div>' + 
					...
					'<div>price: ' + project.projectPrice + '</div>' +
			...
			$('#' + donateContainerId).click(function() {
				...
				loadDonate(categoryId, projectId);
			});				
	...
	var loadFaq = function(categoryId, projectId) {
		var faqSubmit = 'faqSubmit_' + projectId;
		$('#option').html('');		
		
		$.get('option/faq/' + projectId, function(faqs, projectId) {
			...
			$('#option').append('<form id="askQuestion"><textarea name="question" />'+
							'<input type="submit" value="Ask question"></form>');
			
			$('#askQuestion').submit(function(projectId) {
				var enteredText = $(this).find('textarea').val();
				sendAjax(enteredText, projectId);
				
				// will pass the form data using the jQuery serialize function
				//$.post('faq/add', $(this).serialize(), function(response) {
	...
	var sendAjax = function (enteredText, projectId) {
		$.ajax({ 
		url: "faq/add/",  //Путь который мы указываем в контроллере 
		type: 'POST', 
		dataType: 'json', 
		data: "{\"question\":\"" + enteredText +"\"}", 
	...
	$('#test').submit(function(e) {
				// will pass the form data using the jQuery serialize function
				$.post('categories/test', $(this).serialize(), function(response){
					$('#info-cont').append(response);
				});
				e.preventDefault(); 
			});
});